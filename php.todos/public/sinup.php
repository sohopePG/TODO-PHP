...
if(isset($_POST['btn_submit'])){
	...
	//ここでデータベースに登録する
	try{
		$sql = "INSERT INTO user_date (name,password,mail,status,created_at,updated_at) VALUES (:name,:password_hash,:mail,1,now(),now())";
        $stm = $pdo->prepare($sql);
		$stm->bindValue(':name', $_SESSION['name'], PDO::PARAM_STR);
		$stm->bindValue(':mail', $_SESSION['mail'], PDO::PARAM_STR);
		$stm->bindValue(':password_hash', $_SESSION["password"], PDO::PARAM_STR);
		$stm->execute();
...
本登録されました。
<a herf="https://tech-base.net/tb-230072/todo/public/home.php">ホームへ</a>
...
<?php elseif (isset($_POST['btn_confirm']) && count($errors) === 0): ?>
	<form action="<?php echo $_SERVER['SCRIPT_NAME'] ?>?urltoken=<?php print $urltoken; ?>" method="post">
		<p>メールアドレス：<?=h($_SESSION['mail'], ENT_QUOTES)?></p>
...
<!-- page_1 登録画面 -->
   ...
   <?php endif; ?>
		<?php if(!isset($errors['urltoken_timeover'])): ?>
			<form action="<?php echo $_SERVER['SCRIPT_NAME'] ?>?urltoken=<?php print $urltoken; ?>" method="post">
				<p>メールアドレス：<?=h($mail, ENT_QUOTES, 'UTF-8')?></p>
